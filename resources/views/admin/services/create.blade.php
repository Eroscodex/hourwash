<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <!-- Header Title -->
        <div>
            <h1 class="text-xl sm:text-2xl lg:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Add New Service Package</h1>
            <p class="text-xs sm:text-sm text-slate-500 dark:text-zinc-400 mt-1">Create a new laundry package option for store and online bookings.</p>
        </div>
            ...
            <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              class="w-full bg-slate-50 dark:bg-zinc-800/80 border border-slate-300 dark:border-zinc-700 rounded-md px-3.5 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-zinc-100 placeholder:text-slate-400 dark:placeholder:text-zinc-500 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition shadow-sm">{{ old('description') }}</textarea>
                    @error('description') 
                        <p class="text-rose-500 text-xs font-semibold mt-1">{{ $message }}</p> 
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="service_type" 
                                    ...
                                    required>
                                <option value="wash_dry" {{ old('service_type') === 'wash_dry' ? 'selected' : '' }}>Wash & Dry</option>
                                <option value="wash" {{ old('service_type') === 'wash' ? 'selected' : '' }}>Wash Only</option>
                                <option value="dry" {{ old('service_type') === 'dry' ? 'selected' : '' }}>Dry Only</option>
                                <option value="fold" {{ old('service_type') === 'fold' ? 'selected' : '' }}>Fold Only</option>
                                <option value="wash_dry_fold" {{ old('service_type') === 'wash_dry_fold' ? 'selected' : '' }}>Wash, Dry & Fold</option>
                                <option value="blanket" {{ old('service_type') === 'blanket' ? 'selected' : '' }}>Comforters & Blankets</option>
                                <option value="pickup_delivery" {{ old('service_type') === 'pickup_delivery' ? 'selected' : '' }}>Pickup & Delivery</option>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="price_unit" 
                                    ...
                                    class="w-full bg-slate-50 dark:bg-zinc-800/80 border border-slate-300 dark:border-zinc-700 rounded-md px-3.5 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-zinc-100 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition shadow-sm appearance-none pr-10" 
                                    required>
                                <option value="load" {{ old('price_unit', 'load') === 'load' ? 'selected' : '' }}>Per Load</option>
                                <option value="kg" {{ old('price_unit') === 'kg' ? 'selected' : '' }}>Per Kilogram (kg)</option>
                                <option value="item" {{ old('price_unit') === 'item' ? 'selected' : '' }}>Per Item</option>
                                <option value="service" {{ old('price_unit') === 'service' ? 'selected' : '' }}>Per Service</option>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-slate-500">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="status" 
                                    ...
                                    class="w-full bg-slate-50 dark:bg-zinc-800/80 border border-slate-300 dark:border-zinc-700 rounded-md px-3.5 py-2.5 text-xs sm:text-sm text-slate-900 dark:text-zinc-100 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition shadow-sm appearance-none pr-10" 
                                    required>
                                <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Active (Available)</option>
