...
<body class="bg-slate-200 text-slate-900 min-h-screen w-full flex flex-col items-center justify-start p-2 sm:p-3 overflow-y-auto select-none">
    ...
    <div class="printable-card w-[48mm] max-w-[48mm] min-h-[72mm] bg-white px-[2.5mm] py-2.5 text-black space-y-2 text-[9.5px] leading-snug border-0 shadow-sm rounded-none font-sans shrink-0">
        ...
        <div class="text-center space-y-0.5 border-b-2 border-black pb-2">
            ...
            <p class="text-[9px] font-bold text-black leading-tight">Laundry Shop System</p>
            <p class="text-[8.5px] font-semibold text-black leading-tight">Magallanes St., Orosite, Legazpi</p>
            <p class="text-[8.5px] font-bold text-black">Mobile: 09123456789</p>
        </div>

        <!-- Receipt Order Meta -->
        <div class="space-y-1.5 border-b-2 border-black pb-2 text-[9.5px] text-black">
            <div class="flex justify-between items-start gap-1">
                <span class="font-bold text-black shrink-0">RECEIPT:</span>
                <span class="font-black text-right font-mono text-[10.5px] text-black">#{{ $order->order_number }}</span>
            </div>
            <div class="flex justify-between items-start gap-1">
            ...
            <div class="flex justify-between items-start gap-1">
                ...
                <span class="font-black text-right text-black block break-words flex-1 pl-1 text-[10px] leading-tight uppercase">{{ $customerName }}</span>
            </div>
            <div class="flex justify-between items-start gap-1">
            ...
            <div class="flex justify-between items-start gap-1">
                ...
                <span class="font-black text-right text-black block break-words flex-1 pl-1 text-[10px] leading-tight uppercase">{{ $processorName }}</span>
            </div>
            <div class="flex justify-between items-start gap-1">
                <span class="font-bold text-black shrink-0">MACHINE:</span>
                <span class="font-extrabold font-mono text-[9px] text-right text-black">{{ $machineLabel }}</span>
            </div>
        </div>

        <!-- Receipt Line Items -->
        <div class="space-y-1.5 border-b-2 border-black pb-2 text-[9.5px] text-black">
            <div class="flex justify-between font-black border-b-2 border-black pb-1.5 mb-1.5 text-[9.5px]">
            ...
            <div class="flex justify-between items-start pt-1 pb-1 gap-1">
                <div class="flex-1 pr-1">
                    <span class="font-black block leading-snug text-[9.5px] text-black break-words">{{ $serviceName }}</span>
                    <span class="text-[8.5px] font-bold text-black block mt-1 leading-normal">{{ $order->weight_kg }} kg @ ₱{{ number_format($servicePrice, 2) }}/kg</span>
                </div>
                <span class="font-black text-[10.5px] text-black shrink-0 text-right leading-normal">₱{{ number_format($order->subtotal, 2) }}</span>
            </div>

            @if($order->delivery_fee > 0)
                <div class="flex justify-between text-[9px] font-bold text-black pt-1">
        ...
        <div class="space-y-1 border-b-2 border-black pb-2 text-[9.5px] text-black">
            <div class="flex justify-between font-black items-center pt-1">
                ...
                <span class="text-[13px] font-black text-black">₱{{ number_format($order->total_amount, 2) }}</span>
            </div>
            <div class="flex justify-between text-[9.5px] font-extrabold items-center pt-1 pb-0.5">
                <span>PAYMENT:</span>
        ...
        <div class="text-center pt-2 space-y-1.5 text-black">
            <div class="w-24 h-24 mx-auto bg-white p-1 border-2 border-black rounded-none flex items-center justify-center shadow-none">
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=400x400&data={{ urlencode($qrToken) }}&margin=1" 
                     alt="Order QR Tag {{ $order->order_number }}" 
                     crossorigin="anonymous"
                     class="w-full h-full object-contain"
                     style="image-rendering: pixelated; image-rendering: -moz-crisp-edges; image-rendering: crisp-edges;">
            </div>
            <p class="text-[9px] font-bold text-black pt-1 leading-normal">Scan QR Code tag to track order</p>

            <div class="pt-0.5 flex flex-col items-center justify-center space-y-0.5 pb-1">
                <p class="text-[9.5px] font-extrabold text-black leading-tight">Thank you for washing with HourWash!</p>
            </div>
        </div>
