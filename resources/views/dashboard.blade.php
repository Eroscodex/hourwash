            ...
            <div class="app-card p-4 sm:p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
                    @forelse($machines as $machine)
                               ...
                               title="Click anywhere on box to view your order #{{ $targetOrder->order_number }}">
                                ...
                                <div>
                                    ...
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                                        ...
                                        @else
                                            <span class="text-amber-600 dark:text-amber-400 font-semibold">Order {{ ucfirst(str_replace('_', ' ', $targetOrder->order_status)) }}</span>
                                        @endif
                                        <span class="block text-[9px] font-bold text-blue-600 dark:text-blue-400 mt-1 group-hover:text-blue-700">
                        ...
                        @else
                            <div class="p-3.5 rounded-lg bg-black/5 dark:bg-[#18181B] border border-black/5 dark:dark:border-zinc-700 space-y-2 opacity-90">
                                <div class="flex items-center justify-between">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-zinc-700 text-slate-900 dark:text-slate-200">
                            @forelse($recentOrders as $order)
                                <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition">
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider @if($order->order_status === 'completed') bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 @elseif($order->order_status === 'finish') bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/30 @else bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-500/30 @endif">
